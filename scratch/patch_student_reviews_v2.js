...
// The marker we are looking for: the end of the ebook catalog section
...
// We want to insert BEFORE the marker, but after the closing divs of the previous section.
// Let's find the position after the div that ends the ebook grid.
// Line 6140 is the end of the section.
// We'll search backwards from markerPos for the third </div>

let pos = markerPos;
let foundDivs = 0;
while (pos > 0 && foundDivs < 2) {
...
if (foundDivs < 2) {
    console.error('Could not find enough closing divs');
    process.exit(1);
}

// Position is at the start of the second to last </div> (which closes the grid)
// No, line 6140 closes the section. 6139 closes the grid.
// Let's just insert it right before the marker and adjust the style.
...
// Insert it BEFORE line 6141
const lines = html.split('\n');
lines.splice(6140, 0, fullHtml);
